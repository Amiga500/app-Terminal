    ...
    if (event->key.type == SDL_KEYDOWN && event->key.state == SDL_PRESSED) {
        if (show_help) {
            ...
            if (!(selected_j == 3 && selected_i == 0)) {
                ...
                simulate_key(keys[shifted][selected_j][selected_i], toggled[selected_j][selected_i] ? STATE_DOWN : STATE_UP);
            ...
            if (mod_state & KMOD_CTRL) {
                ...
                if (key == SDLK_CAPSLOCK) {
                    toggled[4][0] = shifted = !shifted;
                    update_modstate(SDLK_LSHIFT, shifted ? STATE_DOWN : STATE_UP);
                }
                else {
